...
export function RutFeedPlayer() {
  ...
  return (
    ...
    >
      {customImageUrl && <div className="pointer-events-none absolute inset-0 z-0 bg-black/70" aria-hidden="true" />}
      ...
      <header className="z-30 w-full max-w-md mx-auto px-5 pt-3 pb-1 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex h-8 w-8 items-center justify-center rounded-full border border-zinc-800 bg-zinc-950/70" title="Country identification">
            <svg viewBox="0 0 32 32" className="h-5 w-5 fill-amber-500/90" role="img" aria-label="United Kingdom profile location">
              <path d="M8 4 14 3l3 3 6-1 2 5-3 3 1 5-4 2-1 7-5-2-2-5-5-2 1-5-3-3 3-3Z" />
            </svg>
          </div>
        </div>
      </header>
      ...
      <main className="relative flex-1 flex flex-col justify-between px-5 py-2 max-w-md mx-auto w-full overflow-hidden select-none">
        {isLoadingFeed ? (
          ...
          <>
            <div className="w-full text-left mt-1 pr-14">
              <div className="flex items-center gap-1.5 text-xs sm:text-sm font-semibold text-rose-400">
                <span>{currentRut.category_id === 'cat_love' ? 'Love & Relationships' : currentRut.category_name || 'Deep Late Night'}</span>
              </div>
            </div>

            <div className="relative flex-1 flex flex-col items-center justify-center my-auto py-2">
              <button type="button" onClick={() => setIsAuthOpen(true)} className="group relative cursor-pointer" aria-label={`Open ${currentRut.author_name}'s profile`}>
                {isPlaying && <>
                  <span className="absolute -inset-2 rounded-full border border-amber-400/40 animate-ping pointer-events-none" />
                  <span className="absolute -inset-5 rounded-full border border-amber-500/30 animate-voice-ripple pointer-events-none" />
                  <span className="absolute -inset-8 rounded-full border border-amber-500/15 animate-voice-ripple pointer-events-none" style={{ animationDelay: '0.7s' }} />
                </>}
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={currentRut.author_avatar || `https://api.dicebear.com/7.x/bottts/svg?seed=${currentRut.author_uid}`} alt={currentRut.author_name} className="w-32 h-32 sm:w-36 sm:h-36 rounded-full object-cover border-2 border-zinc-700 shadow-2xl transition-transform group-hover:scale-105" />
                <span className="absolute inset-0 m-auto w-12 h-12 rounded-full bg-black/45 backdrop-blur-xs flex items-center justify-center text-white shadow-lg">
                  {isPlaying ? <Pause className="w-5 h-5 fill-current" /> : <Play className="w-5 h-5 fill-current ml-0.5" />}
                </span>
              </button>

              <h2 className="text-base sm:text-lg font-bold text-white mt-4 tracking-tight text-center">{currentRut.author_name}</h2>
              <p className="text-xs text-zinc-400 font-mono mt-0.5 text-center">{formatTime(duration || currentRut.duration_seconds)} rut</p>
              <h1 className="max-w-[19rem] text-center text-lg sm:text-xl font-extrabold text-white tracking-tight leading-snug mt-4">&ldquo;{currentRut.question_title}&rdquo;</h1>
              <p className="text-[10px] font-bold tracking-widest text-zinc-500 uppercase mt-2">QUESTION</p>
              <button id="more-answered-pill" onClick={() => setIsQuestionsOpen(true)} className="mt-3 px-3.5 py-1.5 rounded-full bg-zinc-900/90 hover:bg-zinc-800 border border-zinc-800 text-xs text-zinc-300 flex items-center gap-2 shadow-sm transition-all active:scale-95 cursor-pointer">
                <div className="flex -space-x-1.5 overflow-hidden"><span className="inline-block w-4 h-4 rounded-full ring-1 ring-zinc-900 bg-amber-200" /><span className="inline-block w-4 h-4 rounded-full ring-1 ring-zinc-900 bg-sky-300" /></div>
                <Users className="w-3.5 h-3.5 text-zinc-400" /><span className="text-[11px] font-medium">2 more answered</span>
              </button>
            </div>

            <div className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/3 flex flex-col items-center gap-3 z-30 rounded-full border border-zinc-800/80 bg-black/35 p-1.5 backdrop-blur-md">
              <button onClick={() => setIsAuthOpen(true)} className="h-10 w-10 rounded-full flex items-center justify-center text-zinc-200 hover:bg-zinc-800 cursor-pointer" title="Open profile"><User className="w-5 h-5" /></button>
              <div className="flex flex-col items-center gap-1"><button id="like-rut-btn" onClick={handleToggleLike} className="relative w-10 h-10 rounded-full bg-zinc-900/90 hover:bg-zinc-800 border border-zinc-800 flex items-center justify-center text-white transition-all active:scale-95 cursor-pointer" title="Like Rut"><Heart className={`w-5 h-5 ${currentRut.user_liked ? 'fill-rose-500 text-rose-500' : 'text-zinc-200'}`} /></button><span className="text-[10px] font-mono text-zinc-400">{formatCount(currentRut.likes_count)}</span></div>
              <div className="flex flex-col items-center gap-1"><button id="comments-rut-btn" onClick={() => setIsQuestionsOpen(true)} className="relative w-10 h-10 rounded-full bg-zinc-900/90 hover:bg-zinc-800 border border-zinc-800 flex items-center justify-center text-white cursor-pointer" title="View responses"><MessageSquare className="w-5 h-5 text-zinc-200" /><span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-amber-500 text-zinc-950 text-[10px] font-bold flex items-center justify-center">3</span></button><span className="text-[10px] font-mono text-zinc-400">134</span></div>
              <div className="flex flex-col items-center gap-1"><button id="share-rut-btn" onClick={handleShare} className="w-10 h-10 rounded-full bg-zinc-900/90 hover:bg-zinc-800 border border-zinc-800 flex items-center justify-center text-white cursor-pointer" title="Share Rut"><Share2 className="w-5 h-5 text-zinc-200" /></button><span className="text-[10px] font-mono text-zinc-400">89</span></div>
              <button onClick={() => setIsMuted(!isMuted)} className="h-10 w-10 rounded-full flex items-center justify-center text-zinc-300 hover:bg-zinc-800 cursor-pointer" title={isMuted ? 'Unmute background music' : 'Mute background music'}>{isMuted ? <VolumeX className="w-5 h-5" /> : <Music className="w-5 h-5" />}</button>
              <button onClick={() => setIsThemeOpen(true)} className="h-10 w-10 rounded-full flex items-center justify-center text-amber-400 hover:bg-zinc-800 cursor-pointer" title="Customize background"><Palette className="w-5 h-5" /></button>
            </div>
      ...
      <footer className="z-40 w-full bg-black/95 border-t border-zinc-900 px-5 pt-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] select-none backdrop-blur-md">
